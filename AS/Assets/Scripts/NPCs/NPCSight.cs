...
public class NPCSight : MonoBehaviour
{
	...
	void OnTriggerStay (Collider other)
	{
		...
		if(other.gameObject == NPC)
		{
			...
			int playerLayerZeroStateHash = NPCAnim.GetCurrentAnimatorStateInfo(0).nameHash;
			int playerLayerOneStateHash = NPCAnim.GetCurrentAnimatorStateInfo(1).nameHash;
			
			// If the player is running or is attracting attention...
			if(playerLayerZeroStateHash == hash.locomotionState || playerLayerOneStateHash == hash.shoutState)
			{
				// ... and if the player is within hearing range...
				if(CalculatePathLength(NPC.transform.position) <= col.radius)
					// ... set the last personal sighting of the player to the player's current position.
					personalLastSighting = NPC.transform.position;
			}
		}
	}
